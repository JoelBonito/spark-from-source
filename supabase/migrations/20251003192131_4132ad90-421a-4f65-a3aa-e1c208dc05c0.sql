...
-- Adicionar colunas à tabela budgets existente
...
DO $$ BEGIN
  DROP POLICY IF EXISTS "Users view own leads" ON public.crm_leads;
  CREATE POLICY "Users view own leads" ON public.crm_leads FOR SELECT USING (auth.uid() = user_id);
EXCEPTION WHEN OTHERS THEN NULL; END $$;
...
DO $$ BEGIN
  DROP POLICY IF EXISTS "Users delete own leads" ON public.crm_leads;
